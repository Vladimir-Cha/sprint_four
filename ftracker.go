...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	var walkCalories float64
	meanSpeedSec := meanSpeed(action, duration) * kmhInMsec
	walkCalories = ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedSec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkCalories
}
